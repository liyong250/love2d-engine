...
local dir = ...
local path = love.util.libPath(dir)
...
-- fonts
dir = string.gsub(dir, '%.', '/')
pixel = Data.font.ch18

local smallfont = pixel -- love.graphics.newFont(dir .. "/assets/fonts/OpenSans-Regular.ttf", 21)
local normalfont = pixel -- love.graphics.newFont(dir .. "/assets/fonts/OpenSans-Regular.ttf", 22)
local boldfont  = pixel-- love.graphics.newFont(dir .. "/assets/fonts/OpenSans-Bold.ttf", 22)
local imagebuttonfont = pixel -- love.graphics.newFont(dir .. "/assets/fonts/OpenSans-Regular.ttf", 24)

-- hakz!
loveframes.basicfontsmall = smallfont
loveframes.textfont = smallfont
...
-- menu
skin.controls.menu_body_color 						= basecolor

-- menu options
skin.controls.menuoption_body_hover_color			= bordercolor:modup(15)
...
function skin.DrawText(object)
	...
	love.graphics.setColor {255, 255, 255, 255}
	love.graphics.setFont(object:GetFont())
	love.graphics.printf(object:GetText(), object:GetX(), object:GetY() + object:GetHeight() / 2, 
		object:GetWidth(), 'center')
...
function skin.DrawMenuOption(object)
	...
	local twidth = loveframes.textfont:getWidth(text)
	...
	if option_type == "divider" then
		...
		love.graphics.setFont( loveframes.textfont )
		local text_x = x + (width - twidth) / 2
		local text_y = y + (height - smallfont:getHeight()) / 2
		if hover then
			-- 底色
			...
			love.graphics.rectangle( 'fill', x + width - w, y, w, height )
			-- 写字
			...
			-- 两边的绿色小方块和底下的垫底
			local w = 3
			love.graphics.setColor( 50, 100, 50, 255 )
			love.graphics.rectangle( 'fill', x, y, w, height )
			love.graphics.rectangle( 'fill', x + width - w, y, w, height )
			-- 写字
